...
Amazon product pages list a single featured offer in their static HTML. The full list of sellers is loaded separately by JavaScript after the page loads, which means CheerioCrawler (which only fetches the raw HTML, without running JavaScript) cannot see it.

If you need all seller offers, you would have to use [PlaywrightCrawler](../../puppeteer_playwright/index.md), which runs a real browser and can wait for that content to load. For this course, scraping the featured offer is enough to cover the key concepts.
